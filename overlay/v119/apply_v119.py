...
# 4) Proofreading: keep it conservative. Preserve spoken numbers and fillers.
# Keep only narrow cleanup for obvious ASR stutters/exact duplicated phrases.
...
new_pipeline = '''        if (final && text.isNotBlank()) {\n            // Conservative cleanup: preserve spoken numbers, fillers and natural emphasis.\n            // Only obvious word stutters and exact duplicated multi-word ASR phrases are collapsed.\n            knownTranscriptionFixes.forEach { (from, to) -> text = text.replace(from, to) }\n            text = removeImmediateWordStutters(text)\n            text = collapseRepeatedPhrases(text)\n            text = restoreFinalPunctuation(text)\n        }'''
...
old_repeats = 'private val intentionalRepeats = setOf("نه", "کم", "تک", "یواش", "آروم", "دونه", "قدم")'
new_repeats = 'private val intentionalRepeats = setOf("نه", "کم", "تک", "یواش", "آروم", "دونه", "قدم", "خیلی")'
if s.count(old_repeats) != 1:
    raise SystemExit(f'Expected intentionalRepeats declaration once; found {s.count(old_repeats)}')
s = s.replace(old_repeats, new_repeats, 1)
...
if 'private val givenNameAliases' not in s:
    if anchor not in s:
        raise SystemExit('PersianNameCorrector warmUp anchor missing')
